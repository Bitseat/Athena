...
wake_word = ['hi Athena', 'hello Athena', 'hey Athena']
greetings = ['hey there', 'hello', 'hi', 'Hai', 'hey!', 'hey']
question = ['How are you?', 'How are you doing?']
responses = ['Okay', "I'm fine"]
jokes = ['Knock knock', 'Who is there?', 'Why don’t scientists trust atoms? Because they make up everything.', 'How do you drown a hipster? Throw him in the mainstream.']
    ...
    if speech_type.lower() != "speech":
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                audio_playback('I did not get that. Rerun the code')
                engine.runAndWait()
    if translate in wake_word:
        # jokrep = random.choice(greetings)
        audio_playback('Hello there, how can i help you?')
        engine.runAndWait()
    elif translate in greetings:
        random_greeting = random.choice(greetings)
        print(random_greeting)
        audio_playback(random_greeting)

    elif translate.lower() == "yes":
        change_location = True
        print("Location?")
        audio_playback('Location?')
        ...
        print(reply)
        audio_playback(reply)
    elif translate in cmd9:
        print(random.choice(repfr9))
        audio_playback(random.choice(repfr9))
        ...
        audio_playback('I am a bot, silly')
        engine.runAndWait()
        ...
        audio_playback(w.get_wind())
        engine.runAndWait()
        audio_playback('humidity')
        engine.runAndWait()
        audio_playback(w.get_humidity())
        engine.runAndWait()
        audio_playback('temperature')
        engine.runAndWait()
        audio_playback(w.get_temperature('celsius'))
        engine.runAndWait()
    elif translate in var3:
        print("Current date and time : ")
        print(now.strftime("The time is %H:%M"))
        audio_playback(now.strftime("The time is %H:%M"))
        engine.runAndWait()
    elif translate in cmd1:
        webbrowser.open('www.google.com')
    elif translate in cmd3:
        jokrep = random.choice(jokes)
        audio_playback(jokrep)
        engine.runAndWait()
        ...
        audio_playback("Where do you want to go:")
        engine.runAndWait()
    else:
        audio_playback("please wait")
        engine.runAndWait()
        print(wikipedia.summary(translate))
        audio_playback(wikipedia.summary(translate))
        engine.runAndWait()
